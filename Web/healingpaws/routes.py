...
def appointment_date_page(date):
    ...
    return render_template(L('appointments.html'),username=session.get("username"),
...
def pets_page():
    ...
    return render_template(L("pets.html"),username=session.get("username"),pets=getUserPets(session.get('username')),rand=rand())
...
def introduction_page():
    ...
    b=success_info
    success_info=None
    return render_template(L("introduction.html"),username=session.get("username"),error_info=a,success_info=b)

    
@app.route("/introductiont")
def introductiont_page():
    global error_info
    a=error_info
    error_info=None
    ...
    b=success_info
    success_info=None
    return render_template(L("cn/introduction.html"),username=session.get("username"),error_info=a,success_info=b)

@app.route("/doctors")
def doctors_page():
    return render_template(L("doctors.html"),username=session.get("username"),users=getAllDoctor(),rand=rand())
@app.route("/users")
def users_page():
    return render_template(L("users.html"),username=session.get("username"),users=getAllUser(),rand=rand())
...
@app.route("/locations")
def location_page():
    return render_template(L("locations.html"),username=session.get("username"))
...
def chat_page1(touser):
    return render_template(L("chat1.html"),username=session.get("username"),chatto=touser)
...
def chat_page(touser):
    if(request.method=="POST"):
        ...
        if(request.form.get("content")):
            chat(fromuser,touser,request.form.get("content"))
        return get_chat(fromuser,touser)
    return render_template(L("chat.html"),username=session.get("username"),chatto=touser)
...
@app.route("/cover")
def cover_route():
	return render_template(L("cover.html"),username=session.get("username"))
